...
- Three hotel suggestions across budget tiers
- A tailored packing checklist (custom feature, see §6)
...
- Frontend: Vercel, root directory `frontend`, env var `VITE_API_URL` = your Render backend URL + `/api`
- Backend: Render, root directory `backend`, build `npm install`, start `npm start`, env vars from `.env.example` (set `CLIENT_ORIGIN` to your Vercel URL)
...
Key design choice: all LLM calls live in one `services/llmService.js` module. Routes and controllers never call the Gemini API directly — they call `llm.generateTripPlan(...)` or `llm.regenerateDay(...)`. This means the AI provider or model can be swapped (e.g. Gemini → OpenAI or Claude) by editing one file.
...
- `frontend/src/components/` holds the pieces the trip detail page composes — `DayCard`, `BudgetCard`, `PackingList`, `HotelsSection`, `DeleteConfirmModal` — each owns its own local UI state where it makes sense (e.g. `DayCard` manages its own "add activity" input), so the parent page stays focused on data-fetching and orchestration rather than micromanaging every input's state.
...
Trip: userId (indexed, ref User), destination, numDays, budgetType, interests[],
      ...
      hotelSuggestions: [{ name, tier, note }],
...
| POST | `/api/trips` | Create trip + generate full AI plan |
...
Trade-off: JWT in `localStorage` is simpler to implement under a 3-day deadline than httpOnly cookies + refresh-token rotation, but it is more exposed to XSS than a cookie-based approach. Documented under Known Limitations.
...
Both functions force strict JSON output via prompt instructions, then parse with a retry-once fallback if the model wraps the response in markdown or returns malformed JSON — this was the most common real failure mode I hit while testing.

---

## 7. Creative Feature: AI-Generated Packing Checklist

Every itinerary stops at "what to do" — none of them tell you what to actually bring. I added a packing checklist generated in the same call as the itinerary, tailored to the destination's likely climate/season, trip length, and the user's stated interests (e.g. "Adventure" pulls in different gear than "Shopping"). It renders as a simple checkbox list on the trip page.

**Why this, and not something else:** it reuses the existing LLM service pattern (no new infrastructure), it's genuinely useful (packing needs vary a lot by destination), and it's easy to demo clearly in a short video — which mattered given the time constraints of this assessment.
...
- **Component decomposition on the trip detail page** — `TripDetail.jsx` composes `DayCard`, `BudgetCard`, `PackingList`, `HotelsSection`, and `DeleteConfirmModal` rather than rendering everything inline, so each piece is independently readable and reusable.
...
- LLM output quality varies with the underlying model; the free-tier Gemini model used during development is sometimes slower or less consistent than a paid model. The retry-once JSON parser mitigates malformed responses but doesn't guarantee itinerary quality.
